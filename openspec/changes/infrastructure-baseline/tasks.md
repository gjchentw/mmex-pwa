...
- [x] 6.8 Verified 2026-07-18 after the `fix-wasm-path-resolution` deploy: the production database opens from OPFS (`SQLite Status: Ready` on a fresh profile, zero console errors), the WASM MIME error is gone, and the service worker registers and serves the app after reload. The manifest/installability half was verified under task 6.7
